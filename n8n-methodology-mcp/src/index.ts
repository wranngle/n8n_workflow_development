...
// Find skills directory - supports both plugin structure (skills/) and project structure (.claude/skills/)
function getSkillsDir(): string {
  const pluginPath = path.join(PROJECT_ROOT, 'skills');
  const projectPath = path.join(PROJECT_ROOT, '.claude', 'skills');
  
  if (fs.existsSync(pluginPath)) return pluginPath;
  if (fs.existsSync(projectPath)) return projectPath;
  return pluginPath; // default to plugin structure
}
  ...
  async ({ skillName }) => {
    ...
    if (!fs.existsSync(skillPath)) {
      ...
      const altPaths = [
        ...
        path.join(skillsDir, skillName.replace(/-/g, '_'), 'SKILL.md')
      ];
      ...
      const availableSkillsDir = getSkillsDir();
      let availableSkills: string[] = [];
      if (fs.existsSync(availableSkillsDir)) {
        availableSkills = fs.readdirSync(availableSkillsDir, { withFileTypes: true })
...
server.registerTool(
  ...
  async () => {
    ...
    if (!fs.existsSync(skillsDir)) {
      return {
        content: [{ type: 'text', text: `Skills directory not found. Checked: ${skillsDir}` }],
