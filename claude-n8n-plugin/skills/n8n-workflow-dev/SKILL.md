...
**For any workflow with complexity ≥ MODERATE (not trivial/simple):**

| Query Type | Minimum Sources | Requirement |
|------------|-----------------|-------------|
| Workflow Design | 25 | Unique, diverse sources across all knowledge bases |
| API/Integration Question | 25 | Official docs + community + templates + examples |
| Diagnostic/Debugging | 25 | Error patterns + community Q&A + similar issues |

### What Counts as a "Source"?

Each of these counts as 1 source (must be unique):
- 1 YouTube video (from video-index.json)
- 1 Discord Q&A (from discord-questions.json)
- 1 n8n template (from mcp__n8n-mcp__search_templates)
- 1 community workflow (from Zie619/n8n-workflows)
- 1 API documentation page (from Context7, Ref, or Exa)
- 1 n8n node documentation (from mcp__n8n-mcp__get_node_documentation)
- 1 Reddit thread
- 1 WebSearch/WebFetch result

### Diversity Requirement

The 25 sources MUST span AT LEAST 4 different categories:
```
✅ VALID: 8 YouTube + 6 Discord + 5 Templates + 4 API Docs + 2 Community = 25
❌ INVALID: 25 YouTube videos (not diverse)
```

### Tracking Format

Document sources in the research report:
```markdown
## Research Sources ({N}/25 minimum)

| # | Type | Source | Relevance |
|---|------|--------|-----------|
| 1 | YouTube | "Webhook Tutorial" by n8n | Node setup pattern |
| 2 | Discord | Q: "How to handle auth..." | Error solution |
| ... | ... | ... | ... |
```

**EXCEPTION**: Trivial/simple workflows (single trigger → single action, no auth) may proceed with fewer sources if explicitly noted.
