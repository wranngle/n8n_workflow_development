...
## AI Node Validation (269 AI-Capable Nodes)

### Critical: AI Connection Direction
...
|------|---------|--------------|
| `ai_languageModel` | LLM provider | OpenAI Chat Model |
| `ai_tool` | Tool for agent | HTTP Request, Database |
| `ai_memory` | Conversation context | Window Buffer Memory |
| `ai_chain` | Chain connections | Conversation Chain |
| `ai_embedding` | Vector embeddings | OpenAI Embeddings |
| `ai_document` | Document loaders | Document Loader |
| `ai_outputParser` | Parse outputs | JSON Output Parser |
| `ai_retriever` | Retrieval | Vector Store Retriever |

### Common AI Validation Errors

```javascript
// Error: "AI tool should connect TO AI Agent, not FROM it"
// ❌ Wrong
{
  "AI Agent": {
    "main": [[{ "node": "HTTP Request Tool" }]]
  }
}
...
```javascript
// Error: "AI Agent has no connected language model"
// Missing required ai_languageModel connection

// Fix: Connect an LLM node to the agent
{
  "OpenAI Chat Model": {
...
### AI Node Validation Profile

Use `ai-friendly` profile for AI workflows:
```javascript
validate_workflow({
  workflow: {...},
  options: {
    profile: "ai-friendly"  // Reduces false positives for AI configs
  }
})
```
