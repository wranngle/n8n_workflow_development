...
## MANDATORY RESEARCH QUOTA: 25 Sources Minimum

**For any research task with complexity ≥ MODERATE:**

| Query Type | Minimum Sources | Diversity |
|------------|-----------------|-----------|
| Integration Research | 25 | ≥4 source types |
| API Documentation | 25 | ≥4 source types |
| Diagnostic Analysis | 25 | ≥4 source types |

### Source Types (count 1 each):
- n8n node documentation (mcp__n8n-mcp__get_node_documentation)
- n8n templates (mcp__n8n-mcp__search_templates)
- API docs (Context7, Ref-tools, Exa)
- YouTube tutorials (video-index.json)
- Discord Q&A (discord-questions.json)
- Community workflows (Zie619/n8n-workflows)
- Reddit threads
- WebSearch results

### Tracking Requirement

Include in every research report:
```markdown
## Research Sources ({N}/25 minimum)

| # | Type | Source | Finding |
|---|------|--------|---------|
| 1 | Node Docs | nodes-base.slack | Auth flow |
| 2 | Template | #1234 | Pattern |
| ... | ... | ... | ... |
```

**EXCEPTION**: Simple queries with obvious answers may use fewer sources if explicitly justified.
...
## Writing to Research Library

After generating a research report, you MUST update the research library index for reuse by other pipelines (ai_sales_engineering, wranngle-proposal-generator).

### Step 1: Save the Report

Save your markdown report to:
```
context/technical-research/{slug}.md
```

Where `{slug}` is a kebab-case version of the business process (e.g., `sync-ringcentral-calls-to-pipedrive`).

### Step 2: Update the Library Index

Read and update `context/technical-research/library-index.json`:

```javascript
// Read existing index
Read("context/technical-research/library-index.json")

// Add new entry to research_files
{
  "research_files": {
    "{slug}": {
      "file": "{slug}.md",
      ...
      "complexity_score": N,
      "effort_tier": "trivial|moderate|complex|critical",
      "confidence": 0.XX
    }
  },
  "integration_lookup": {
...
Recalculate and update the `stats` section:
```json
{
  "stats": {
    "total_reports": N,
    "integrations_covered": N,
...
The library index enables ai_sales_engineering to:
1. Look up cached research by integration name
2. Check freshness (created_at)
3. Use complexity scores for estimates
4. Skip research for already-documented integrations

**IMPORTANT**: Always update the index after creating or updating research. Orphaned reports are useless to downstream pipelines.
