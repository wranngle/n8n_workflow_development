...
## ⚠️ CRITICAL: AI Connection Direction (REVERSE PATTERN)

**AI connections flow TO the AI Agent node, not FROM it!**

This is the OPPOSITE of standard n8n node connections:

```
Standard nodes:  Source → Target  (data flows right/down)
AI tool nodes:   AI Agent ← Tool  (tool connects TO agent)
```

**Why?** AI tools are attached to the agent as capabilities, not as data flow destinations.

### Correct Connection Pattern

```javascript
// In connections object, AI tools connect TO the agent:
{
  "connections": {
    "HTTP Request Tool": {
...
```javascript
// ❌ WRONG - Connecting agent OUTPUT to tool
{
  "AI Agent": {
    "main": [[{ "node": "HTTP Request Tool", "type": "main", "index": 0 }]]
  }
}

// ✅ CORRECT - Tool connects TO agent via AI port
{
  "HTTP Request Tool": {
    "ai_tool": [[{ "node": "AI Agent", "type": "ai_tool", "index": 0 }]]
  }
...
The n8n-mcp validation tools check for correct AI connection direction. Common validation errors:
- "AI tool should connect TO AI Agent, not FROM it"
- "Connection type mismatch: expected ai_tool"
- "AI Agent has no connected language model"
